...
impl<T> myStack<T> {
    ...
    pub fn push(&mut self, elem: T) {
        // Push to the non-empty queue; if both empty, push to q1
        if !self.q1.is_empty() {
    ...
    pub fn pop(&mut self) -> Result<T, &'static str> {
        ...
        if !self.q1.is_empty() {
            ...
            self.q1.dequeue() // pop the last one
        } else {
            // Move all but last element from q2 to q1
            while self.q2.size() > 1 {
                let val = self.q2.dequeue().unwrap();
                self.q1.enqueue(val);
            }
            self.q2.dequeue() // pop the last one
        }
    }
